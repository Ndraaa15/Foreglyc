...
func (h *FoodHandler) GenerateDietaryInformation(ctx *fiber.Ctx) error {
	c, cancel := context.WithTimeout(ctx.UserContext(), 5*time.Second)
	defer cancel()
	...
	res, err := h.foodService.GenerateDietaryInformation(c, userId)
	if err != nil {
		h.log.WithError(err).Error("failed to get dietary information")
		return err
	}
